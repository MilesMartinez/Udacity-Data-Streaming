...
stediEventsJSONSchema = StructType(
    [
        StructField('customer', StringType()),
        StructField('score', StringType()),
...
spark = SparkSession.builder.appName('STEDI-Risk-App').getOrCreate()
...
rawRedisServerDataStreamDF = spark.readStream\
    .format('kafka')\
...
RedisServerDataStreamDF = rawRedisServerDataStreamDF.selectExpr('cast(value as string) value')
...
RedisServerDataStreamDF.withColumn('value', from_json('value', redisMessageSchema))\
    .select(col('value.key'),\
            col('value.existType'),\
            col('value.ch'),\
            col('value.incr'),\
...
RedisDataSelectDF = spark.sql('select key, zsetEntries[0].element as customer from RedisSortedSet')
RedisDecodedDataSelectDF = RedisDataSelectDF.withColumn('customer', unbase64(RedisDataSelectDF.customer).cast('string'))
...
RedisDecodedDataSelectDF.withColumn('customer', from_json('customer', customerJSONSchema))\
    .select(col('customer.*'))\
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
RawStediEventsStreamDF = spark.readStream\
    .format('kafka')\
    .option('kafka.bootstrap.servers', 'localhost:9092')\
    ...
    .load()                                   
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediEventsStreamDF = RawStediEventsStreamDF.selectExpr('cast(value as string) value')
...
stediEventsStreamDF.withColumn('value', from_json('value', stediEventsJSONSchema))\
    .select(col('value.*'))\
...
joinedDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr('customer = email'))
...
# joinedDF.writeStream.outputMode('append').format('console').start().awaitTermination()

joinedDF.selectExpr('cast(customer as string) key', 'to_json(struct(*)) as value')\
    .writeStream\
    .format('kafka')\
    .option('kafka.bootstrap.servers', 'localhost:9092')\
